...
export function VendorOrdersContent({ orders, vendorId }: VendorOrdersContentProps) {
  ...
  return (
    <div className="space-y-4">
      {localOrders.map((order) => {
        ...
        const vendorItems = orderItems.filter((item: Record<string, unknown>) => {
        ...
        const canUpdate = ['pending', 'confirmed', 'preparing'].includes(status)
              ...
              <div className="flex items-center gap-3">
                {canUpdate && (
                  <select
                    ...
                    className="text-sm px-3 py-1.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#8FBC8F] focus:border-transparent outline-none disabled:opacity-50"
